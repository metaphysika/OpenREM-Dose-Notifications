...
def send_dx_high_dose_alert_email(study_pk=None, test_message=None, test_user=None):
    """
    Function to send users a radiography high dose alert e-mail
    """
    from remapp.models import GeneralStudyModuleAttr
    from django.core.mail import EmailMultiAlternatives
    from django.template.loader import render_to_string
    from openremproject import settings
    from openremproject.custom_settings import DX_ALERT_RECIPIENTS, DX_EI_ALERT_VALUE
    from socket import error as socket_error
    from socket import gaierror as gai_error
    from smtplib import SMTPException
    from ssl import SSLError
    ...
    # Get the irradiation event data
    irrad_event = study.projectionxrayradiationdose_set.get().irradeventxraydata_set.all()
    eis = [e.irradeventxraydetectordata_set.get().exposure_index for e in irrad_event]

    #  Check if EI is above alert_level threshold
    if any([ei >= DX_EI_ALERT_VALUE for ei in eis]):
        content_dict = {
            'study': study,
            'alert_level': DX_EI_ALERT_VALUE,
            'this_study_ei': max(eis),
            'server_url': settings.EMAIL_OPENREM_URL
        }

        text_msg_content = render_to_string('remapp/dx_dose_alert_email_template.txt', content_dict)
        html_msg_content = render_to_string('remapp/dx_dose_alert_email_template.html', content_dict)
        recipients = DX_ALERT_RECIPIENTS
        msg_subject = 'OpenREM high dose alert: Radiography EI'
        msg = EmailMultiAlternatives(msg_subject, text_msg_content, settings.EMAIL_DOSE_ALERT_SENDER, recipients)
        msg.attach_alternative(html_msg_content, 'text/html')
        msg.send()
...
def send_ct_high_dose_alert_email(study_pk=None, test_message=None, test_user=None):
    ...
    from django.template.loader import render_to_string
    from openremproject import settings
    from openremproject.custom_settings import CT_ALERT_RECIPIENTS, CT_CTDI_ALERT_VALUE
    from socket import error as socket_error
    from socket import gaierror as gai_error
    from smtplib import SMTPException
    from ssl import SSLError
    ...
    # Get the irradiation event data
    ctirrad_event = study.ctradiationdose_set.get().ctirradiationeventdata_set.all()
    ctdis = [e.mean_ctdivol for e in ctirrad_event]

    # Check if EI is above alert_level threshold
    if any([ctdi >= CT_CTDI_ALERT_VALUE for ctdi in ctdis]):
        content_dict = {
            'study': study,
            'alert_level': CT_CTDI_ALERT_VALUE,
            'this_study_ctdi': max(ctdis),
            'server_url': settings.EMAIL_OPENREM_URL
        }

        # Create string with exam info. Include study desc., proc. desc., req. proc. desc., acc #, etc.

        # if study_desc not in customsettings.IGNORE_CT_STUDY
        #   send the email
        #   append to string "sent"
        # else
        #   append to string "not sent"
        # write string to customsettings.LOG
        ...
        recipients = CT_ALERT_RECIPIENTS
        msg_subject = 'OpenREM high dose alert: CT CTDI'
        msg = EmailMultiAlternatives(msg_subject, text_msg_content, settings.EMAIL_DOSE_ALERT_SENDER, recipients)
        msg.attach_alternative(html_msg_content, 'text/html')
        msg.send()